...
public class TableMaskCellEditor extends JTextField implements TableCellEditor, DocumentListener, ActionListener, CustomPaintSite, java.io.Serializable {
	...
	public boolean isCellEditable(EventObject anEvent) {
		if (anEvent instanceof MouseEvent) {
			if (((MouseEvent) anEvent).getClickCount() > 1) { return true; }
		...
		if (ems != null) {
			...
			if (formatter != null) {
				try {
					...
					try {
						/*
						 * if (formatter.getFormatObj() instanceof java.text.DateFormat) { oldPattern = formatter.getPattern(); int yearMaskIndex = -1; if ((yearMaskIndex =
						 * DBUtilities.yearMaskPos(oldPattern)) != -1) { int yearMaskCount = 1; int patternLength = oldPattern.length(); while ((yearMaskIndex +
						 * yearMaskCount) < patternLength && oldPattern.charAt(yearMaskIndex + yearMaskCount) == 'y') { yearMaskCount++; } if (yearMaskCount < 4) {
						 * widePattern = oldPattern.substring(0, yearMaskIndex) + "yyyy" + ((yearMaskIndex + yearMaskCount < patternLength) ?
						 * oldPattern.substring(yearMaskIndex + yearMaskCount) : ""); } } if (widePattern != null) { formatter.setPattern(widePattern); } }
						 */
	...
	private void handleKeyEvent(KeyEvent e) {
		...
		if (!acceptKeyReleasedEvent && id != KeyEvent.KEY_PRESSED && id != KeyEvent.KEY_TYPED) { return; }
