...
/**
 * <p>
 * A data-aware extension to the <code>JList</code> component. <code>JdbList</code> displays a list of choices from which a user may choose a single value. The
 * selected value is written to a column of a <code>DataSet</code> and retrieved using the standard Swing <code>ListSelectionModel</code> methods. When
 * <code>JdbList</code> is bound to a <code>DataSet</code> and <code>DataSet Column</code>, the currently selected choice always reflects the value of the
 * corresponding column in the current <code>DataSet</code> row.
 * </p>
 *
 * <p>
 * There are several ways to fill the list presented by <code>JdbList</code>:
 * </p>
 *
 * <ul>
 * <li>Set <code>JdbList</code>'s <code>items</code> property with an array of <code>String</code>s or <code>Object</code>s to display a static list of
 * choices.</li>
 * <li>Use the standard <code>JList</code> model methods to populate the list from a <code>Vector</code>, <code>Object</code> array, or any
 * <code>ListModel</code> implementation.</li>
 * <li>Retrieve the list of choices from another <code>DataSet</code> by setting the <code>picklist</code> property on <code>JdbList</code>'s
 * <code>columnName</code> <code>Column</code>. The choices displayed by <code>JdbList</code> are always kept in synchronization with changes in the source
 * <code>DataSet</code>. By specifying the <code>picklist</code> property accordingly, it's possible to display a value from one column but save a value from a
 * different column or even save the values from more than one column.</li>
 * <li>Instantiate a <code>DBListDataBinder</code> and set its properties to connect it with <code>JdbList</code> and to a <code>JListModel</code> and
 * <code>JListSelectionModel</code>.</li>
 * </ul>
 *
 * <p>
 * You should avoid setting both the column's <code>picklist</code> property and <code>JdbList</code>'s <code>items</code> property on the same list. If both
 * properties are set in the UI Designer, the <code>picklist</code> setting takes precedence at runtime. When the list is displayed, however, the most recently
 * set property setting always takes precedence.
 * </p>
 *
 * <p>
 * Note that because a list can display one column only at a time, if more than one column is specified as a display column in the picklist definition, only the
 * first display column is displayed by <code>JdbList</code>. For more information about picklists, see <code>com.borland.dx.dataset.PickListDescriptor</code>
 * class.
 * </p>
 *
 * <p>
 * Ordinarily, the selection in a data-aware <code>JdbList</code> is the current value of the column it is bound to. If this value is not in its list, a
 * <code>JdbList</code> has no selected value by default, but you can modify this behavior by changing the value of the <code>unknownDataValueMode</code>
 * property.
 * </p>
 *
 * @see JdbNavList
...
public class JdbList extends JList
	...
	/**
	 * <p>
	 * Constructs a <code>JdbList</code> component by calling the constructor of its superclass.
	 * </p>
	 */
	public JdbList() {
		super();
	...
	/**
	 * <p>
	 * Constructs a <code>JdbList</code> component by calling the constructor of its superclass and passing to it a <code>ListModel</code>.
	 * </p>
	 *
	 * @param dataModel
	 *          The <code>ListModel</code> that contains the data in the list displayed by <code>JdbList</code>.
	 *
	 */
	...
	/**
	 * <p>
	 * Constructs a <code>JdbList</code> component by calling the constructor of its superclass that takes an array of <code>Object</code>s as a parameter.
	 * </p>
	 *
	 * @param listData
	 *          An array of <code>Object</code>s that makes up the list displayed in the <code>JdbList</code>.
	 */
	...
	/**
	 * <p>
	 * Constructs a <code>JdbList</code> component by calling the constructor of its superclass that takes a <code>Vector</code> as a parameter.
	 * </p>
	 *
	 * @param listData
	 *          A <code>Vector</code> that makes up the list displayed in the <code>JdbList</code>.
	 */
	...
	/**
	 * <p>
	 * Used to initialize <code>JdbList</code> with the same defaults, regardless of the constructor used. A newly instantiated <code>JdbList</code> differs from
	 * a <code>JList</code> in that it always has SINGLE_SELECTION mode as its <code>selectionMode</code>.
	 * </p>
	 *
	 * <p>
	 * Also, the default cell renderer for a <code>JdbList</code> is a <code>JdbList.DBCellRenderer</code> rather than a DefaultListCellRenderer.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Sets the list of items to select from. Setting this property replaces the existing model from which the list is built.
	 * </p>
	 *
	 * @param items
	...
	/**
	 * <p>
	 * Sets a static array of <code>Objects</code> to display in the list. Setting this property replaces the existing model from which the list is built.
	 * </p>
	 *
	 * @param items
	 *          The list of items to select from.
	...
	/**
	 * <p>
	 * Returns the items displayed in the list as an array of <code>Strings</code>.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Sets a <code>DataSet</code> from which the current list selection is read and to which the current list selection is written.
	 * </p>
	 *
	 * @param dataSet
	...
	/**
	 * <p>
	 * Returns the <code>DataSet</code> from which the current list selection is read and to which the current list selection is written.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Sets the name of a <code>Column</code> in the <code>DataSet</code> from which the current list selection is read and to which the current list selection is
	 * written. Setting this property replaces the current <code>ListModel</code> from which the list is built.
	 * </p>
	 *
	 * @param columnName
	 *          The name of a <code>Column</code> in the <code>DataSet</code>.
	 * @see #getColumnName
	 */
	...
	/**
	 * <p>
	 * Returns the name of a <code>Column</code> in the <code>DataSet</code> from which the current list selection is read and to which the current list selection
	 * is written.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Sets the policy for synchronizing a list selection with its <code>DataSet</code> value when the value can't be found in the list.
	 * </p>
	 *
	 * @param mode
	 *          Possible values are:
	...
	/**
	 * <p>
	 * Returns the policy for synchronizing a list selection with its <code>DataSet</code> value when the value can't be found in the list.
	 * </p>
	 *
