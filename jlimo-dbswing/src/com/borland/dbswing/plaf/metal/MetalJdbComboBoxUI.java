...
public class MetalJdbComboBoxUI extends MetalComboBoxUI {
  ...
  protected ComboPopup createPopup() {
    MetalJdbComboPopup popup = new MetalJdbComboPopup(comboBox);
  ...
  protected ComboBoxEditor createEditor() {
    if (useLookAheadComboBoxEditor) {
      return new BasicJdbComboBoxEditor(comboBox, new EditorBorder());
    }
    ...
    public void show() {
      ...
      if (dropDownWidth != -1) {
        ...
        if (isShowTable()) {
          ...
          popupWidth += insets.left + insets.right + scrollBarWidth;
        }
      }
        ...
        if (isVisible()) {
          ...
          if (newEvent.getPoint().y >= r.y && newEvent.getPoint().y <= r.y + r.height - 1) {
            ...
            if (hasEntered) {
              ...
              if (e.getPoint().y < 0) {
                hasEntered = true;
//                MetalJdbComboPopup.this.startAutoScrolling(BasicComboPopup.SCROLL_UP);
...
//            else if (keyNav.equals("Heavyweight")) {
      ...
      public void keyReleased(KeyEvent e) {
        if (!comboBox.isEditable() && isVisible()) {
