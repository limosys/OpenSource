...
public class WindowsJdbComboBoxUI extends WindowsComboBoxUI {

  public static ComponentUI createUI(JComponent c) {
    return new WindowsJdbComboBoxUI();
  }

  public void installUI( JComponent c ) {
    myComboBox = (JComboBox) c;
    super.installUI( c );
  }

  protected ComboPopup createPopup() {
    WindowsJdbComboPopup popup = new WindowsJdbComboPopup(comboBox);
  ...
  public class WindowsJdbComboPopup extends WindowsComboBoxUI.WindowsComboPopup {
...
//    protected int scrollDirection = BasicComboPopup.SCROLL_UP;
    ...
    public WindowsJdbComboPopup(JComboBox cBox) {
      super(cBox);
/**
 * This is a workaround for a Java bug.(BTS 71921)
 * Overloaded function createKeyListener() is renamed to createKeyListener1(),
 * createListSelectionListener() to createListSelectionListener1().
 * Defer installation of some listeners until super's construction is completed.
 */
...
//        Object keyNav = WindowsJdbComboBoxUI.this.myComboBox.getClientProperty("JComboBox.lightweightKeyboardNavigation");  
        if (keyNav != null) {
...
//              table.getSelectedRow() != WindowsJdbComboBoxUI.this.myComboBox.getSelectedIndex() &&
//              table.getSelectedRow() < WindowsJdbComboBoxUI.this.myComboBox.getItemCount() &&
        ...
        if (e.getKeyCode() == KeyEvent.VK_UP && !e.isAltDown()) {
          if (isVisible()) {
            ...
            if (isShowTable()) {
              ...
              if (getJList().getSelectedIndex() > 0) {
                getComboBox().setSelectedIndex(getJList().getSelectedIndex()-1);
              }
            }
          }
          e.consume();
          ...
          if (isVisible()) {
            ...
            if (isShowTable()) {
              ...
              if (getJList().getSelectedIndex() < (getJList().getModel().getSize() - 1)) {
                getComboBox().setSelectedIndex(getJList().getSelectedIndex()+ 1);
              }
            }
          }
...
//          index = WindowsJdbComboBoxUI.this.myComboBox.getModel().getSize() - 1;
        }
      }
...
//            table.setModel((TableModel) WindowsJdbComboBoxUI.this.myComboBox.getModel());
            configureTable();
...
//                                                ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER));
  ...
  protected JComboBox myComboBox; // workaround for package-protected JDK 1.2.2 problem
