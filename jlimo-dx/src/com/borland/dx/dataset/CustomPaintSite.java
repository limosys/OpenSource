...
/**
 * The ItemPaintSite interface is the one that is passed to the Column.CustomPaint event handler. Classes that implement the ItemPaintSite and CustomPaintSite
 * interfaces can provide item painters with information about the host container in which the painting occurs.
 * <p>
 * The CustomPaintSite interface has the methods for retrieving and setting display-related properties, such as the background color, the foreground color, the
 * font, the alignment setting, the margins for the item being painted, and the item's transparent state.
 */
public interface CustomPaintSite {
	 ...
	 * 
	 * @param border
	 *          A javax.swing.border.Border object to be used instead of default border.
	 */
	...
	/**
	 * Returns the component representing the ItemPaintSite. This is used for coordinate space calculations, as well as to provide a component for ItemPainter
	 * implementations that require one - like ImageItemPainter, which requires an ImageObserver object.
	 * 
