...
/**
 *
 * The <CODE>dx.dataset</CODE> package contains classes and interfaces that provide basic data access.  This package also defines base provider and resolver classes as well as an abstract <CODE>DataSet</CODE> class that is extended to other <CODE>DataSet</CODE> objects.  These classes provide access to information stored in databases and other data sources.
<P>
This package includes functionality covering the three main phases of data handling:
<P><BTBL><STRONG>Data handling</BTBL></STRONG>
<P>
<TABLE CELLSPACING=0 CELLPADDING=4 BORDER=1>

<TR>
<TD VALIGN=TOP>Providing</TD>
<TD VALIGN=TOP>General functionality to obtain data and manage local data sets. (JDBC specific connections to
remote servers are handled by classes in the <CODE>com.borland.dx.sql.dataset</CODE> package.)</TD>
</TR>
<TR>
...
<TD VALIGN=TOP>General routines for the updating of data from the local <CODE>DataSet</CODE> back to the original source of the data. (Resolving data changes to remote servers through JDBC is handled by classes in the <CODE>com.borland.dx.sql.dataset</CODE> package.)</TD>
</TR>
</TABLE>

<BR>
The <CODE>DataSet</CODE> class is an abstract class that provides basic editing, view, and
cursor functionality for access to two-dimensional data. It supports the concept of a current row
position which allows for navigation of the data in the <CODE>DataSet</CODE>. The <CODE>DataSet</CODE> also
manages a <CODE>pseudo</CODE> record -- an area in memory where
a newly inserted row or changes to the current row are temporarily stored.

<P>The data in a <CODE>DataSet</CODE> can be modified programmatically or through a data-aware control.
To connect a <CODE>DataSet</CODE> to a data-aware
control, set the control's <CODE>dataSet</CODE> property to the <CODE>DataSet</CODE> you want to use
as the data source. Several data-aware controls can be associated with the same <CODE>DataSet</CODE>.
...
the same <CODE>DataSet</CODE>. This synchronization of controls that share a common <CODE>DataSet</CODE>
can greatly ease the development of the user-interface portion of your application.

<P>Controls which share the same <CODE>DataSet</CODE> as their data source share
...
<P>The <CODE>DataSet</CODE> component is opened implicitly (by default) when visual components bound to it are shown. For example, launching an application that includes a data-aware control that is bound to a <CODE>DataSet</CODE> automatically opens the <CODE>DataSet</CODE> so you seldom have to open the <CODE>DataSet</CODE> explicitly. No code is generated for this implicit open.

<P>You can navigate through the data in a <CODE>DataSet</CODE> using a UI control or programmatically. To navigate programmatically, use the
<CODE>first()</CODE>, <CODE>last()</CODE>, <CODE>next()</CODE>, and <CODE>prior()</CODE> methods. To verify if such a navigation is valid use the <CODE>inBounds()</CODE> method. Similarly, to perform tests for the "end of file" or "beginning of file" conditions, use the <CODE>atLast()</CODE> or <CODE>atFirst()</CODE> methods.

<!-- JDS start - remove last sentence of paragraph -->

<P>The <CODE>DataSet</CODE> has an associated
{@link com.borland.dx.dataset.SortDescriptor SortDescriptor}
 object that defines properties which affect the sort order of the
data in the <CODE>DataSet</CODE>. This is accessed from the <CODE>sort</CODE> property in the JBuilder Inspector.
<!-- JDS end -->

<A NAME="masterdetail"></A>
<P><CODE>DataSet</CODE> objects have relational capability and
can be linked to form master-detail relationships. The
{@link com.borland.dx.dataset.MasterLinkDescriptor MasterLinkDescriptor} holds the
properties required for relational capability between <CODE>DataSet</CODE> objects.
<!-- JDS start - remove sentence -->
For a tutorial using the MasterLinkDescriptor, see "Establishing a master-detail relationship" in the <CITE>Database Application Developer's Guide</CITE>.
<!-- JDS end -->


<P>The <CODE>DataSet</CODE> class is extended by
{@link com.borland.dx.dataset.DataSetView DataSetView} and
{@link com.borland.dx.dataset.StorageDataSet StorageDataSet}. The <CODE>DataSetView</CODE> component allows for an alternate view (sort order, navigation, and filter criteria) for the data  contained in the <CODE>DataSet</CODE>.  A <CODE>StorageDataSet</CODE>, and any class that extends <CODE>StorageDataSet</CODE>, manages the storage aspects of the data operated on by the <CODE>DataSet</CODE>.

 * An abstract class to provide a cursor for accessing and navigating
 * table data. Manages a pseudo-record in memory to temporarily store
 * a newly inserted row, or changes to the current row.
 * Multiple data-aware controls can be bound to and synchronized with
 * the same DataSet. Supports master-detail relationships.
 * Supports model-view data-aware controls, to enable easy and flexible
 * editing and navigation of data in a common way regardless of how the
 * data was obtained.
 */
  ...
  implements StatusListener, MasterNavigateListener, AccessListener, Designable
{
  DataSet() {
    ...
    * <p>
    * Also, when a Provider, such as the QueryProvider used by QueryDataSet,
    * is loading rows asynchronously, attempting to access the DataSet data or
   ...
   */
   ...
   /**
    *
    * @return    <b>true</b> if the current position is the last
    *             visible row of DataSet, otherwise, this method returns false.
    *             This method is the equivalent of the EOF() (beginning of file)
    *             function that is available in many programming languages.
    */
  ...
  /**
   * Is this dataSet at the first row visible by this dataSet?
   * No need for synchronization.
   */

   /**
    * @return <b>true</b> if the current position is the first visible row
    *         of DataSet, otherwise, this method returns <b>false</b>.
    *         This method is the equivalent of the BOF() (beginning of file)
    *         function that is available in many programming languages.
    */
  public final boolean  atFirst() { return currentRow == 0; }

   /**
    * Moves the row position to the first row visible to the DataSet
    * and sends a NavigationEvent to registered NavigationListeners
    * if the move was successful. This may cause the {@ link #inBounds()}
  ...
  public final void first() /*-throws DataSetException-*/ {
   ...
   /**
    *  Moves the current position to the last row visible to the DataSet
    *  and sends a NavigationEvent notification to registered
    *  NavigationListeners if the move was successful.
    *  This may cause the inBounds() to return true if more than one row exists.
    *  <p>
    *  If an edit is in progress, the changes are posted prior to performing
    *  the move operation. On failure, this method throws a DataSetException.
    */
  public final void last() /*-throws DataSetException-*/ {
   ...
   /**
    * Moves the row position to the next row visible to the DataSet and sends
    ...
    *
    * @return <b>true</b> if the row position could be moved to the next row visible
    *         to the DataSet.
    */
  public final boolean next() /*-throws DataSetException-*/ {
   ...
   /**
    * Moves the row position to the previous row visible to the DataSet and
    * sends a NavigationEvent notification of rowChanged to its registered
    * NavigationListeners if the move was successful. This will cause the
    * inBounds() method to return <b>false</b> if {@link #prior()} is called when the DataSet
    * is positioned at the first visible row.
    *
    * @return   <b>true</b> if the row position could be moved to the previous
    *           row visible to the DataSet.
    */
...
//!    }
   ...
   /**
    * Start editing a new row. The before parameter indicates whether to
    * insert before or after the current row.
    * <p>
    * This method adds a new, unposted "pseudo" row into the DataSet.
    * Until post() is called, this row will not be visible to DataSetView
    * components that are sharing the same StorageDataSet. After insertRow() has
    * been called, values can be set in the pseudo row by calling DataSet set methods.
    * The insertRow() method provides the functionality of row inserting in data aware
    * visual components.
    * <p>
    * When the post() method is called, the unposted row is stored in the StorageDataSet
    * and becomes visible by DataSetViews sharing the same StorageDataSet.
    * When the row is posted, and the sort property is not set, the current row in the
    * DataSet will change to the newly-posted row at the end of the DataSet.
    * If the DataSet sort property is set, the current row of the DataSet will
    * be the newly-posted row, displayed in its sorted position within the DataSet.
    * <p>
    * Many navigating methods will cause an unposted row to be automatically posted.
    * In this case, the new row appears to "fly away" to its proper position,
    * which may be at the end of current rows if the sort property is <b>null</b>, or to
    * its sorted position if the sort property is not <b>null</b>. In these cases, the
    * current row is not necessarily (and is not likely to be) the newly-posted row.
    * Instead the cursor is positioned according to the navigating request.
    * For example, if the prior() method is called while an unposted row exists,
    * the new row will be posted into the StorageDataSet according to the sort property
    * setting, but the current row position will be one row up.
    *
    * @see #addRow(com.borland.dx.dataset.DataRow) for higher performance batch row adding.
    *
    * @param before   Whether the new row should be inserted before (<b>true</b>),
    *                 or after (<b>false</b>), the current row. This property defaults to <b>false</b>.
    */
  ...
  final void _editRow() {
   ...
   /**
    *  Locates the row of data with the specified row of values and moves
    *  the current row position to that row. The locate operation includes
    *  all columns of the ReadRow; to limit the locate to specific columns
    *  of interest, use a scoped DataRow. If the row is scoped to a specific set
    *  of columns, only those columns are used for the locate.
    *
    * @param rowLocate        The ReadRow that contains values to use in for
    *                         the locate operation.
    * @param locateOptions    Options that are applicable when locating data.
    *                         Valid values for the locateOptions are defined
    *                         in {@link com.borland.dx.dataset.Locate} variables.
    *                         The Locate variables may be combined using the
    *                         Java bit-wise OR operator of a vertical pipe symbol (|)
    *                         between each variable where it makes sense to do so.
    *                         For example, you can search using partial strings
    *                         and specifying case insensitivity.
    * @return                 <b>true</b> if the row of data containing the
    *                         specified values were located.
    */
  public final boolean locate(ReadRow rowLocate, int locateOptions) /*-throws DataSetException-*/ {
    ...
    *  and returns <b>true</b> but does not navigate to the matching row. If no match
    *  is found, this method returns <b>false</b>. This method includes all columns of
    *  the ReadRow; to limit the lookup to specific columns, use a "scoped"
    *  DataRow that includes only the columns of interest.
    *
    * @param locateRow      The ReadRow that contains values to use in for
    *                       the lookup operation.
    * @param resultRow      The data in the row where the match with the
    *                       rowLocate values is found.
    * @param locateOptions  Options that are applicable when locating data.
    *                       Valid values for the locateOptions are defined in
    *                       {@link com.borland.dx.dataset.Locate} variables.
    *                       The Locate variables may be combined using a
    *                       vertical pipe symbol ( | ) between each variable
    *                       where it makes sense to do so. For example, you can
    ...
    /*-throws DataSetException-*/
  {
    ...
    * record to be posted. If this method is called for an unposted row,
    * the row will be posted in the StorageDataSet with a RowStatus of INSERTED.
    * If the sort property is not set, the new row will be positioned at the end
    * of the StorageDataSet. If the sort property is set, the row will be positioned
    * according to the sort property setting.
    * <p>
    * If called for an existing row, the RowStatus for that row is UPDATED,
    * if it does not already have the RowStatus of INSERTED.
    * As with new rows, it will be positioned according to the sort property setting.
    *
...
//!         _fixRowPosition();
  ...
  */
    ...
    /*-throws DataSetException-*/
  {
    if (!open)
      failIfNotOpen();
    if (editing) {
      synchronized(dataMonitor) { _cancel(); }
  ...
  /**
      @updated JB2.0
      Cancels the loading of data into the DataSet.  This can be used to terminate
      a long running asynchronous provider operation.  The QueryProvider used by
      QueryDataSet has Load options that allow it to load query results into
      a StorageDataSet asynchronously on a separate thread.  Calling this method
      will cause such an operation to terminate.
  ...
  */
  ...
  /**
      @since JB2.0
      Cancels any long running operation currently active on this DataSet.  This includes
  ...
  */
    ...
    /*-throws DataSetException-*/
  {
    ...
    * @param resolved   <b>true</b> if changes were resolved and the changed
    *                   rows should now be treated as originals in a new resolution query.
   ...
   *  @param resolved  true  if changes were resolved and the changed rows
   *                         should now be treated as originals in a new
   *                         resolution query.
   *                   false if changes were rolled back and the changed rows
   *                         should still be treated as changed rows.
   */
   ...
   * However, it is completely forgotten and cannot be resolved back to a data source.
   *
   * @see #deleteRow()
   */
  ...
  /**
   *  This method calls the emptyRow() method for all rows visible in the DataSet.
   *  If called on a detail DataSet, only the rows in the current group are emptied.
   *  <p>
   *  The emptied rows are not marked as deleted rows.
   *  However, they are completely forgotten and cannot be resolved back to a data source.
   */
  ...
  /**
   * Deletes the current row of the DataSet.
   * If the current row is new or being edited, the edit state is canceled
   * and the new or edited row is deleted.
  ...
  /**
      ...
      @see emptyAllRows to remove visible rows in the DataSet without marking them
      as deleted.
  */

  /**
   * Deletes all rows of data visible in the DataSet.
   * If called on a detail DataSet, only the rows in the current group are deleted.
   * Deletion status information is tracked for resolution back to the
   * original data source. If all rows of the DataSet cannot be deleted,
   * this method throws a DataSetException.
   *
   * @see #emptyAllRows to remove visible rows in the DataSet without marking them
                        as deleted.
  ...
  /**
   * Assigns the value stored at the intersection of the specified
   * columnName and row to value.
   *
   * @param columnName    The String name of the Column.
   * @param row           The unique row identifier for the row.
   * @param value         The value that is assigned by executing this method.
   */
   ...
   *
   * @param ordinal     An integer value that represents the nth Column in
   *                    the DataSet where the Variant value is located.
   * @param row         A integer value representing the unique row identifier for the row.
   * @param value       The value that is assigned by executing this method.
  ...
  /**
      @since JB2.0
      Called by data aware controls.  Sets value to the value that should be displayed in the visual control.  Normally this is the same
      value that would be retrieved from a call to getVariant().  If the column at the ordinal position has its PickList property set,
      value may be initialized to the value from another column in a separate pickList dataSet.  This provides the capability to store a
      compact "code" value, but display a more "descriptive" value.  This behavior is enabled when the column at position ordinal
      has its CalcType property set to LOOKUP and it has a PickList property setting with lookupDisplayColumn property is set
      to a column name in the PickList pickListDataSet.

      @see Column.PickList
 */
 /**
  *  Called by data-aware controls. Sets the value parameter to the value
  *  that should be displayed in the visual control.
  *  Normally this is the same as what
  *  {@link #getVariant(int, int, com.borland.dx.dataset.Variant)} returns.
  *  If the column at the ordinal position has its pickList property set,
  *  the value parameter may be initialized to the value from another
  *  column in a separate pick list DataSet. This provides the capability
  *  to store a compact "code" value, but display a more "descriptive" value.
  *  This behavior is enabled when the column at position ordinal has its
  *  calcType property set to LOOKUP and it has a defined PickListDescriptor
  *  with its lookupDisplayColumn property set to a column name in the pick
  *  list DataSet.
  *
  * @param ordinal
  * @param row
  ...
  */
  ...
  /**
      @since JB2.0
      Called by data aware controls.  Performs the same operation as
      setVariant() if there is no PickList property with LookupDisplayColumn.

      If the Column at the ordinal position does have a PickList property set with
      LookupDisplayColumn, then value is assumed to be a LookupDisplayColumn value
      and all the corresponding fillin columns are assigned if value can be found
 ...
 */
 ...
 /**
  *  Called by data aware controls. Sets the value parameter to the value
  *  that should be displayed in the visual control. Normally this is the same as what
  *  {@link #getVariant(int, int, com.borland.dx.dataset.Variant)} returns.
  *  <p>
  *  If the column at the ordinal position has its {@link com.borland.dx.dataset.Column}
  *  property set, the value parameter may be initialized to the value from another column
  *  in a separate pick list DataSet. This provides the capability to store a
  *  compact "code" value, but display a more "descriptive" value. This behavior
  *  is enabled when the column at position ordinal has its
  *  com.borland.dx.dataset.Column.calcType property set to LOOKUP and
  *  it has a defined {@link PickListDescriptor} with its lookupDisplayColumn
  ...
  */
  ...
  //
  final Variant[] getRowValues() { return rowValues; }
   ...
   *  {@link com.borland.dx.dataset.Column#validate(DataSet, Variant)} method if it is set.
   *  Note that these checks will be made as data is edited in the DataSet,
  ...
  /*
      ...
      method if it is set.  Note that these checks will be made as data is
      edited in the DataSet and when the changes are saved back to the database
  */
   ...
   *  return true if editing a "new" record.
   */

   // bad name, should be called isInserted.
   //
   /**
    * @param row    The row parameter represents the unique row identifier for the row.
...
//!   */
  ...
  /* Marks a row as pending resolution.  Used internally when saving
      changes from a DataSet to a remote data provider that supports transactions
      (ie JDBC connections)
  */
...
//!  */
  ...
  /**
   * Updates all values in the current row with those in the dataRow.
   * If the DataSet is not open, a DataSetException of DataSetNotOpen is generated.
   * @param dataRow
   */
...
//!       Diagnostic.println("count:  "+count+" "+ordinal);
  ...
  /**
   * Adds a new row to the DataSet at the end of existing rows
   * and sets the new row's RowStatus to INSERTED.
   * The new row is validated against data constraints (if any)
   * to ensure that only valid data is added.
   *
   * @param dataRow   The DataRow to add to the current DataSet.
   */
  ...
  /**
   *  Add a row to underlying dataset.
   *  Like addRow(DataRow) except the internalRow of the row added is returned.
   *
   * @see             #insertRow(boolean before)
   * @param dataRow   The row to be added to the underlying dataset.
    ...
    /*-throws DataSetException-*/
  {
    ...
    * @param row       The unique rowId for a row in the DataSet.
    * @param dataRow   The DataRow object that gets its values from the
    *                  row indicated by the row parameter when this method is executed.
    */
    ...
    /*-throws DataSetException-*/
  {
    ...
    * @param dataRow    The DataRow object that gets its values from the
    *                   current row position when this method is executed.
    */
   ...
   * @param listener The listener for open/close access events.
   */
   ...
   * @param listener The listener for NavigationEvent dispatches.
   */
   ...
   * @param listener The listener for DataChangeEvent dispatches.
   */
  ...
  public final boolean isDisplayErrors() { return displayErrors; }

  /**
   * Instructs all status listeners (for example, the StatusBar control)
   * to clear their status messages.
   */
...
//! */
   ...
   * @param listener The listener for StatusEvent dispatches.
   */
   ...
   * @param listener The listener for MasterNavigate dispatches.
   */
  ...
  public final RowFilterListener getRowFilterListener(){
   ...
   /**
    *  Specifies the SortDescriptor object where properties that define the
    *  sort order of the data in a DataSet are stored. This property is accessed
    *  in the JBuilder Inspector from the sort property.
    *  <p>
    *  Setting the sort descriptor generates a dispatch to the registered
    *  AccessListener object.
    *
    * @return The SortDescriptor object where properties that define the
    *  sort order of the data in a DataSet are stored.
    */
    ...
    /*-throws DataSetException-*/
  {
    ...
    *
    * @param descriptor The SortDescriptor object where properties that define
    *                   the sort order of the data in a DataSet are stored.
    */
...
/**
 * Toggles the view order of the DataSet from ascending to descending
 * (and vice versa) given the values in the specified columnName.
 *
 * @param columnName
 */
  ...
  /**
   *  Affects whether data-aware controls ignore changes or repaint as a
   *  result of programmatic changes to the position and general state of
   *  a DataSet. A control cannot repaint properly while the DataSet events are disabled.
   *  <p>
   *  When performing lengthy DataSet operations, you may want to set this property
   *  to <b>false</b> prior to initiating such operations. If you do, you must set this property
   *  back to <b>true</b> to allow UI controls to repaint properly.
   *  You may also want to execute this method to propagate changes made to a DataSet
  ...
  /**
   * Specifies the most recent Column that was navigated to in a data-aware control.
   * This method is used by the interactiveLocate() method as the default Column
   * to locate on if its parameter of a target Column for the locate is not specified.
   *
   * @param columnName  The most recent Column that was navigated to in a data-aware control.
   */
  ...
  */
   ...
   *
   * @param text                The String representation of the value to search for.
   * @param locateColumnName    The String name of the Column in which to perform
   *                            the search. If null, this method attempts to use
   *                            the last column visited (as returned by
   *                            {@link #getLastColumnVisited()}). If the last column
   *                            visited was not set, the first Column in the
   *                            StorageDataSet is used.
   * @param locateOptions       An integer value representing the locate options
   *                            to use. See {@link com.borland.dx.dataset.Locate} for valid
   *                            values for this parameter.
   * @param enterPressed        Specifies whether the Enter key should be pressed
   *                            prior to initiating the search or if each key press
   *                            should be used to perform an incremental search.
   *                            Incremental searches are available only for String searches.
   */
...
//!       Diagnostic.println(locateColumnName+" lastColumnVisited:  "+lastColumnVisited);
   ...
   *  method and <b>false</b> if the DataSet does not need closing
   *  (for example, it is already closed). Closing a DataSet allows for
   *  structural changes to be made to the DataSet, such as adding a new
   *  Column or changing the sort order, and so on. All changes are applied
   *  to the data currently in the DataSet. The close() method does not
   *  discard any inserted, edited or deleted rows in the DataSet.
   *  For the DataSetView subclass of DataSet, the close() method must
   *  be called when explicitly removing an instantiated DataSetView be
   *  to garbage-collected. Otherwise, the instantiated DataSetView
   *  remains allocated in memory as long as its associated StorageDataSet
   *  cannot be garbage-collected. If you connect to your data source using
   *  a QueryDataSet and its query statement contains parameters, you need to
   *  call DataSet.close() before providing for another QueryDataSet.
   *
   * @return  <b>true</b> if the DataSet is closed by executing this
   *  method and <b>false</b> if the DataSet does not need closing.
   */
  ...
  /**
   * Posts all unposted rows for all DataSet and DataSetView components
   * that share the same StorageDataSet property.
   */
...
//      Diagnostic.printStackTrace();
   ...
   * @return      The method returns <b>true</b> if the DataSet can be successfully opened.
   *              If the DataSet is already open, the method returns <b>false</b>.
   */
  public  boolean open()
    /*-throws DataSetException-*/
  {
...
//            _goToRow(0);
  ...
  /**
      ...
      @return The name of StorageDataSet object that
              manages the storage of the rows that DataSet operates on.
  */
  public final StorageDataSet getStorageDataSet() { return dataSetStore; }
  ...
  /**
   * Throws an exception that has informative error message if editing
   * of column cannot be allowed.
   * @param column
   */
  ...
  /*
      DO NOT MAKE public!  User can access methods off of StorageDataSet
      to get dataSets that have only deleted, changed and inserted
      rows.  Providing this to the user forces any future storage
      implementations to support a broader variety of data delta indexes.
      Set visible rows in the dataSet by row status mask.
      @see RowStatus
  */
...
//!          */
   ...
   /**
    *  Returns whether the specified column can be navigated to (<b>true</b>)
    *  or not (<b>false</b>).
    *
    *  This method returns false if a Column's readOnly property is set to <b>true</b>.
    *  This method is typically used by visual components.
    *
    * @param column   The Column component to navigate to.
    * @param row      The row position in the DataSet.
    * @return         <b>true</b> if the specified column can be navigated to,
    *                 <b>false</b> otherwise.
    */
  public final boolean  canNavigate(Column column, int row) { return !column.readOnly; }

  /**
   * Return current row position.
   * @return The current row position.
   */
  public final int getRow() { return (int)currentRow; }
  public final long getLongRow() { return currentRow; }
   ...
   * @return      The current row position.
   */
  public final int row() { return (int)getLongRow(); }

   /**
    *  Moves to the specified row position, where row represents the unique
    *  row identifier for the row.
    *
    * @param row    The unique row identifier of the row to move to.
    * @return       Returns <b>true</b> if the move is successful,
    ...
    /*-throws DataSetException-*/
  {
    ...
    *
    * @param row    The unique row identifier for the row of the DataSet
    ...
    /*-throws DataSetException-*/
  {
    ...
    if (!open)
      failIfNotOpen();
    synchronized(dataMonitor) { moved = _goToClosestRow(row); }
    if (moved)
      rowNavigatedDispatch();
    return moved;
  }
...
//!*/
  ...
  /**
   *
   * @param code      A status constant from StatusEvent
   * @param message   The message to send to all of this DataSet's StatusListeners
   */
    ...
    /*-throws DataSetException-*/
  {
       ...
       )
    {
      ...
      /*
          Try to avoid notifying the listeners, especially if rowAffected is known (>=0).
          Note that a detail view can have its position set to the first row when this
          dispatch is made.  That can throw off detail iteration logic.  Especially if
          the detail is not affected by the data change.
      */
...
//!           Diagnostic.println("row navigating");
  ...
  */
  /**
   * Returns <b>true</b> if the most recent navigation was in bounds,
   * otherwise, this method returns <b>false</b>. A navigation is in bounds if
   * it falls between the first and last records (inclusive) that are
   * visible to the cursor. This method allows you to test whether either
   * condition of beginning or end of file is encountered with a single method call.
   * <p>
   ...
   * </ul>
   * <p>
   * If you prefer or if your application requires it, you can call any of
   * these methods to reset the <code>inBounds()</code> flag. Any navigation effected through
   * the user-interface also sets <code>inBounds()</code> flag.
   *
   * @return  <b>true</b> if the most recent navigation was in bounds,
   *          otherwise, this method returns <b>false</b>.
   */
  ...
  /**
   * Sets the in bounds state to <b>true</b>.
   * This method is not normally used in an application's code since any
   * successful navigation sets this to <b>true</b>.
   */
    ...
    /*-throws DataSetException-*/
  {
    if (!open)
      failIfNotOpen();
    synchronized(dataMonitor) { isInBounds = true; }
  }
...
//!*/
  ...
  //!      }
    ...
    if (needsRelink) {
      try { first(); }
      catch (DataSetException ex) { DiagnosticJLimo.printStackTrace(ex); }
    }
  }

  /**
   *  Initializes the current row to default values of all columns.
   *  If the DataSet is a detail DataSet of a master-detail relationship,
   *  this method also initializes the detail linking columns.
   */
  ...
  /**
      Initializes the row DataRow to default values of all columns.  If the DataSet is
      a detail DataSet of a master-detail relationship, this method also initializes
      the detail linking columns.

      @param row  The DataRow whose default values for all of its columns
                  will be set.
  ...
  */
  ...
  private long safeRowCount() {
    try{return getLongRowCount();}
    catch(DataSetException ex) {DiagnosticJLimo.printStackTrace(ex);return 0; }
  }
...
//!       Diagnostic.printStackTrace();
  ...
  /**
      Stores the MasterLinkDescriptor object that contains the properties of a master-detail
      relationship.  This proerty is set for the detail DataSet of a master-detail relationship
      and is applicable for any StorageDataSet implementation such as QueryDataSet.
      This property can be be used with a DataSetView component if the MasterLinkDescriptor
      FetchAsNeeded property is not set to true.

      @param descriptor  The masterLink property. The MasterLink property editor in
                          the JBuilder Visual component designer can be used to construct and set
                          a MasterLink property.
  */
...
//!                      + "\r\n  and open state = " + isStorageOpen());
  ...
  /**
      Stores the MasterLinkDescriptor object that contains the properties of a master-detail
      relationship.  This property is set for the detail DataSet of a master-detail relationship
      and is applicable for any StorageDataSet implementation such as QueryDataSet.
      This property can be be used with a DataSetView component if the MasterLinkDescriptor
      FetchAsNeeded property is not set to true.

      @return MasterLinkDescriptor  The MasterLinkDescriptor object that contains the properties of a master-detail
                                    relationship.
  */
...
//!   Diagnostic.println("DataSet.getMasterLink() on " + this + " -->" + masterLink);
   ...
   *
   * @return    The array of all detail DataSets associated
   *            with this master DataSet.
   */
  ...
  /**
   *  Returns the detail DataSet with given tableName property setting.
   *  This method throws a DataSetException.UNKNOWN_DETAIL_NAME if there
   *  is no detail with tableName. It is possible (but not likely)
   *  that more than one detail has the same tableName.
   *
   * @param tableName   The tableName property containing the detail DataSet.
  ...
  /** @since JB2.0
      returns detail dataSet with given TableName property seting.
      returns null if there is no detail with tableName.
      Note that it is possible, but not likely that more than one detail have
      the same tableName.
  */
   ...
   *  <p>
   *  <b>Note:</b> It is possible, but not likely, that more than one
   *  detail has the same tableName.
   *
   * @param tableName
   * @return            The detail DataSet with given tableName property setting or
   *                    null if there is no detail with tableName.
   */
  ...
  /**
   * Call this method on a master DataSet to force all details to open.
   * By default, a detail will ask its master DataSet to open, but a master
   * DataSet does not ask the detail DataSet to open.
   */
  ...
  /**
   * Indicates true if the Column should be displayed in UI controls.
   * This method returns true for Columns that have their visible property set.
   * Aggregate calculated columns and master-detail link columns are hidden by
   * default and return false.
   *
   * @param columnName  The String name of the Column component.
...
//!  */
  ...
  /**
   *  Forces this DataSet to move to the same position as row if row is of
   *  the same StorageDataSet as this DataSet. If row is not from the same
   *  StorageDataSet, this method throws a DataSetException of INCOMPATIBLE_DATA_ROW.
   *  If this DataSet object's current view is filtered and does not include row,
   *   the current position of this DataSet moves to the closest row that is in its view.
   *
   * @param row   The unique row identifier of the row to move to.
   */
...
/**
 * Saves changes made to the data in the DataSet back to the data source
 * using default {@link com.borland.dx.sql.dataset.QueryResolver} behavior.
 */
  ...
  /**
   * Refreshes the data from the DataSet if the data provider supports
   * this operation (for example, QueryDataSet and ProcedureDataSet).
   */
   ...
   *  goToInternalRow method to reposition the current row position.
   *  The internalRow assignment for a row never changes, and is never reused
   *  by another row. Setting this property is the fastest way to navigate to
   *  an arbitrary row.
   *
...
//!       Diagnostic.println("internalRow:  "+index.internalRow+" "+internalRow);
  ...
  /*
     Must be synchronized by dataMonitor.
     Call this when you know the current internalRow is gone (deleted).
     Basically, an internalRow(row) call is more efficient than a findClosest(internalRow).
  */
...
//!       Diagnostic.println("internalRow:  "+index.internalRow+" "+internalRow);
  ...
  /**
   *  Attempts to navigate to newInternalRow.
   *   If newInternalRow no longer exists, position remains unchanged.
...
//!  */
  ...
  /**
   * Refetches the specified row if the DataSet has a data provider
   * (for example, QueryDataSet and ProcedureDataSet) that supports this operation.
   *
   * @param row   The ReadWriteRow implementation that you want refetched.
   */
  ...
  /*  @since JB2.0
      If there is a sort order for this DataSet, the maintained index that maintains
      the sort based on DataSet.Sort property and the DataSet.rowFilter event
      will be dropped.

      Note that if the DataSet is immediately reopened without changing the DataSet.Sort
      property or DataSet.rowFilter event listener, the index will be rebuilt.
  */
   ...
   *  <p>
   *  <b>Note:</b> If the DataSet is immediately reopened without
   *  changing the sort property or rowFilter event listener of this class,
   ...
   * <p>
   * Closing and re-opening a DataSet with a filter will not always cause
   * the filter code to be re-executed. Similarly, if your filter is based
   * on the value of a global variable, changing the global variable will not
   * change the set of rows displayed. In both cases, use the refilter() method
   * to ensure that the filter is re-executed.
   */
  ...
  /**
   * Copies the contents of the previous row to the current (new) row.
   * Does nothing if the current row is the first row. If the current row
   * is an existing row, and you want to copy the contents of the previous
   * row to it, set dittoExisting to true. By default, dittoExisting is
   * false, as copying over the contents of an existing row could cause data loss.
   *
   * @param dittoExisting   If true, copy the contents of the previous
   *                        row to an existing row. By default this
   *                        option is false, as it could cause data loss.
   */
  ...
  /**
   * Copies the contents of the previous row to the current row.
   * Does nothing if the current row is the first row.
   * @param dittoExisting   If <b>true</b>, copies the contents of the previous
   *                        row to an existing row. By default this
   *                        option is <b>false</b>, as it could cause data loss.
   * @param autoInsert      Automatically inserts a new row if the DataSet
   *                        is not in edit mode, then copies the contents
   *                        of the previous row to the new row.
   */
   ...
   * (<b>true</b>) or not (<b>false</b>). For TableDataSet components, this method
   * returns <b>false</b> if the resolver property is not set.
   */
  public boolean saveChangesSupported() { return dataSetStore != null ? dataSetStore.saveChangesSupported() : false; }
   ...
   * @return
   */
  public boolean refreshSupported() { return dataSetStore != null ? dataSetStore.refreshSupported() : false; }
  ...
  /** Disallow row insertion.  If a row insertion is attempted,
      a ValidationException will be thrown.  UI controls have default error
      handling that will show this in a StatusControl if one is present.  If
      there is no StatusControl, the error will be displayed in an error dialog.
      If this property is set on a DataSetView, only that view is affected by this
      setting.
  */
  public final void setEnableInsert(boolean enable) {
    this.allowInsert = enable;
  }
  public final boolean isEnableInsert() { return allowInsert; }

  /** Enable/Disable row update.  If row update is attempted,
      a ValidationException will be thrown.  UI controls have default error
      handling that will show this in a StatusControl if one is present.  If
      there is no StatusControl, the error will be displayed in an error dialog.
      If this property is set on a DataSetView, only that view is affected by this
  ...
  */
  ...
  public final boolean isEnableUpdate() { return allowUpdate; }

  /** Enable/Disable row deletion.  If a delete is attempted,
      a ValidationException will be thrown.  UI controls have default error
      handling that will show this in a StatusControl if one is present.  If
      there is no StatusControl, the error will be displayed in an error dialog.
      If this property is set on a DataSetView, only that view is affected by this
  ...
  */
  ...
  public final boolean isEnableDelete() { return allowDelete; }


  /** Enable/Disable edits to this DataSet.  If an edit operation is attempted,
      a ValidationException will be thrown.  UI controls have default error
      handling that will show this in a StatusControl if one is present.  If
      there is no StatusControl, the error will be displayed in an error dialog.
      If this property is set on a DataSetView, only that view is affected by this
  ...
  */
   ...
   * By default unmodified rows are removed during a post.
   * Setting this property would preserve rows with just default values.
   * @param postUnmodified boolean true if unmodified rows are to be posted.
   */
  ...
  /*  original comments
      ...
      @returns delta moved.  If the index does not have the Sort.SortAsInserted property set,
      this will return 0.  If an attempt is made to move the row out of range,
      the return value will reflect how much of a position change was made.  The row
      will not move out of range.  Range is defined as the first and last row of the
      DataSet when the Sort property has no sort keys.  If the Sort property has sort
      keys, then range is bound by the rows that share the same sort key values.
  */

  /**
   * For DataSets that have the Sort.SortAsInserted property set to true,
   * this method can be used to move the row.
   *
   * @param delta
  ...
  /**
   * @param columnName  The specified columnName.
