...
class PreparedStmt {
  ...
  void resetState(Database database, String query, MasterLinkDescriptor masterDetail, ReadWriteRow parameterRow) /*-throws DataSetException-*/ throws SQLException {
    ReadWriteRow[] rows = parameterRow == null ? null : new ReadWriteRow[]{parameterRow};
    resetState(database, query, masterDetail, rows);
  }

  void resetState(Database database, String query, MasterLinkDescriptor masterDetail, ReadWriteRow[] parameterRows) /*-throws DataSetException-*/ throws SQLException {
...
/*
    ...
    try {
      doClose = !(database.isUseStatementCaching() && database.getMaxStatements() > 10);
    }
    finally {
      if (doClose)
        closeStatement();
...
//!kna  void classifyNamedParameter(String columnName, int param) /*-throws DataSetException-*/ {
  private void classifyNamedParameter(String columnName, int param, char quoteChar) /*-throws DataSetException-*/ {//!kna
    ...
    if (pos > 0) {
      ...
      paramNo--;
      if (paramNo < 0) {
        Column column = (masterDetail == null) ? null :
                         masterDetail.getMasterDataSet().hasColumn(columnName);
        if (column != null) {
