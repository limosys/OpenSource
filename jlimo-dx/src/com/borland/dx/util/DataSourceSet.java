 ...
 *
 * <p>Title: </p>
 * <p>Manager for loading, storing and accessing a working set of JDBC DatSource objects: </p>
 *
 * <p>
 * The working set of DataSources is loaded from a {@link java.util.Properties Properties} <CODE>Object</CODE>.
 * serveral styles of property settings for JDBC DataSource definitions are supported.

 * When used with other
 * DataSource repositories like JNDI, ODBC, and Oracle's tsnames, the
 * {link DataSourceSet DataSourceSet} can be used to expose just all or part of
 * the DataSources available in those repositories.  The DataSourceSet can also
 * be used to associate tool properties with a particular data source such as an
 * url pattern or a boolean that indicates whether JDBC extended properties should
 * be used.  Tool properties are properties that are not needed to make a connection
 * when calling {@link javax.sql.DataSource#getConnection DataSource.getConnection} method.
 * </p>
 * <p>
 * The {@link #load(Properties) load(Properties)} and {@link #store(Properties) store(Properties)} provide
 * persistance to and from a Properties objects for the DataSource instances managed by a
 * DataSourceSet.  The load and store methods manage the persistance of three categories
 * of properties for a DataSource:
 *
 * <p>
 * DataSource properties are stored and loaded from the set of methods on the
 * DataSource implementation that have public accessor methods that begin with
 * "get" and return a value of int, boolean or String.  These accessor methods
 * must not have any arguments and there must also be a setter method that has
 * a single argument with the same type as the return type of the get method.
 * </p>
 *
 * <p>
 * Extended properties are stored and loaded from the Properties object returned
 * from the {@link #getProperties() getProperties()} method.  Note these are only
 * processed if the DataSource class exposes such a method.
 * </p>
 *
 * <p>
 * URL properties are stored and loaded from the Properties object returned
 * from the {@link #getUrlProperties() getUrlProperties()} method.  Note these are only
 * processed if the DataSource class exposes such a method.  The {@link com.borland.javax.sql.JdbcDataSource JdbcDataSource}
 * DataSource implementation uses this Properties objects to process substitution variables in
 * its {@link com.borland.javax.sql.DataSourceProperties#getUrl() JdbcDataSource.getUrl()} property setting.
 * </p>
 *
 * <p>
 * If the DataSource needs to be loaded from an external repository such as JNDI,
 * ODBC, or Oracl tsnames, an additional class that implements {@link DataSourceRepository DataSourceRepository} must
 * be specified when adding the DataSource to the DataSourceSet.  See the
 * {@link #addDataSource addDataSource} method.
 * </p>
 *
...
/*
      ...
      //sync up
      repository.load( dataSource, storeProps );
      repository.store(dataSource, dsProps);
...
/*
      Steve:  Should not be needed anymore
//        dest.put(prefix+(prefix.endsWith(".")?"":key.startsWith(".")?"":".")+key, source.getProperty(key));
        This is more readable if its needed...
...
*/
   ...
   * @param fileName file to load DataSources from.
   */
   ...
   * @param in InputStream to load DataSources from.
   */
  ...
  private final void loadOldDbPilotUrl(Properties props, String key) {
    String s = props.getProperty(key);
    Properties oldProps = new Properties();
    JdbcDataSource dataSource = new JdbcDataSource();
    dataSource.setUser(""); //NORES

    int repCount = 0;
    FastStringBuffer fsb = new FastStringBuffer(s);
    int i = fsb.length() - 1;
    int maxLen = fsb.length();

    // See if this has a "{xxxxx=yyyy}" property list at the end
    // Count the number of trailing '}' chars, since that will
    // be the repeat count for all separators in this string
    if (i >= 0 && fsb.charAt(i) == '}') {
      while (i >= 0 && fsb.charAt(i) == '}') {
        ++repCount;
        --i;
      }

//System.err.println(" repeatCount is " + repCount);

      FastStringBuffer openBrace = new FastStringBuffer('{', repCount);
      FastStringBuffer equals = new FastStringBuffer('=', repCount);
      FastStringBuffer comma = new FastStringBuffer(',', repCount);
      int charPos = fsb.lastIndexOf(openBrace, i);
      if (charPos < 0)
        return;
      maxLen = charPos - 1;
      charPos += repCount;
      int endPos;
      FastStringBuffer name;
      FastStringBuffer value;
      while (charPos < fsb.length()) {
        endPos = fsb.indexOf(equals, charPos);
        name = fsb.substring(charPos, endPos);
//System.err.println("    name is \"" + name + "\" and runs from " + charPos + " to " + endPos);
        charPos = endPos + repCount;
        endPos = fsb.indexOf(comma, charPos);
        if (endPos < 0)
          endPos = fsb.indexOf(new FastStringBuffer("}"), charPos); //NORES
        value = fsb.substring(charPos, endPos);
//System.err.println("    value is \"" + value + "\" and runs from " + charPos + " to " + endPos);
        charPos = endPos + repCount;

        if (name.toString().equalsIgnoreCase("username") && dataSource.getUser().length() == 0) { //NORES
          dataSource.setUser(value.toString());
//System.err.println(" -- set username to " + dataSource.getUserName());
        }
        else {
          Properties properties = (dataSource.getProperties() != null ? dataSource.getProperties() : new Properties());
          properties.put(name.toString().trim(), value.toString().trim());
          dataSource.setProperties(properties);
//System.err.println(" -- properties now " + dataSource.getProperties());
        }
      }

    }
//System.err.println("url/driver is between char 0 and char " + maxLen);
    int driverIndex = s.lastIndexOf("<", maxLen); //NORES
//System.err.println("  spotted driver delimiter at " + driverIndex);
    dataSource.setUrl((driverIndex < 0) ? fsb.toString() : fsb.substring(0, driverIndex-1).toString().trim());
    dataSource.setDriver((driverIndex < 0) ? "" : fsb.substring(driverIndex+1, maxLen-1).toString().trim()); //NORES
    String dataSourceName = dataSource.getUrl();
    dataSource.setDataSourceName(dataSourceName);

    oldProps.put(key+DataSourceProps.OLD_DBPILOT_USE_EXTENDED_PROPS, props.getProperty(key+DataSourceProps.OLD_DBPILOT_USE_EXTENDED_PROPS, "false")); //NORES
    oldProps.put(key, s);
    dataSourceProps.put(dataSourceName, oldProps);
    addDataSource(dataSourceName, dataSource, null);

//System.err.println("parseResult = " + dataSource);
  }
  ...
  public void load(Properties props) {
    ...
    while (enum1.hasMoreElements()) {
      key = enum1.nextElement().toString();
      if (key.startsWith(DataSourceProps.OLD_DBPILOT_URL) && !key.endsWith(DataSourceProps.OLD_DBPILOT_USE_EXTENDED_PROPS)) {
        loadOldDbPilotUrl(props, key);
      }
      else if (key.startsWith(DataSourceProps.DATASOURCE_PREFIX)){
...
//      Diagnostic.println("key:  "+prefixKey);
   ...
   * @param name of the DataSource to obtain a Connection Object for.
   ...
   * @param name of the DataSource to obtain a DataSource Object for.
...
/*
    ...
    DataSource result = setRepository.getDataSource(name);
    if( result != null ){ // SYNC UP
      Properties p = ( Properties ) repositoryProps.get( name );
      ( ( DataSourceRepository ) repositories.get( name ) ).load( result, p );
    }
    return result;
*/
...
//        addDataSource(name, dataSource, repository);
   ...
   * @param name of <code>DataSource</code> to obtain Properties for.
   * @return
   */
  ...
  /**
   * Add a DataSource to this set.  Note that the DataSourceRepository will default
   * to {@link MemoryDataSourceRepository MemoryDataSourceRepository} if null is passed in.
   *
   * @param name unique logical name for the DataSource.
   * @param dataSource DataSource instance to add.
   * @param repository DataSourceRepository implementation to retrieve the DataSource from
   */
  ...
  /**
   * Add a DataSource to this set using a Properties Object.  Note that if DataSourceRepository
   * is null, it will default to {@link MemoryDataSourceRepository MemoryDataSourceRepository}
   *
   * @param name unique logical name for the DataSource.
   * @param props Properties object with DataSource settings.
   * @param repository DataSourceRepository implementation to retrieve the DataSource from
   */
  ...
  public DataSet getConnectionProperties(String dsName) {
    ...
    protocol.setPickList(new PickListDescriptor(getProtocolChoices(),new String[]{PROTOCOL},new String[]{NAME},new String[]{PROTOCOL},NAME,false));
...
//      dataSet.setAssignedNull(ordinal);
  ...
  public void applyChange(String name, DataSet dataSet) {
    ...
    //TODO:  Remove if test when ServerConsole can properly manage multiple DataSource
    // types.
    //
    if (props.get(DataSourceProps.DATASOURCE_CLASS_NAME).toString().equals(com.borland.javax.sql.JdbcDataSource.class.getName())) {
      setProperty(props, CREATE_PROPERTY,        dataSet, CREATE_INDEX);
      setProperty(props, READONLYTX_PROPERTY,    dataSet, READONLYTX_INDEX);
    }
    ...
    if (dataSet.isNull(columnIndex))
      ...
      switch (dataSet.getColumn(columnIndex).getDataType()) {
        ...
        case Variant.BOOLEAN:
          ...
          break;
        default:  DiagnosticJLimo.fail();
      }
    }
  }

  /**
   * Obtain a Hastable for all DataSources in this set.
   * Note that currently this is not a "cloned" copy, and should not be edited.
