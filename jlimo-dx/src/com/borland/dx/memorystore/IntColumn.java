//--------------------------------------------------------------------------------------------------
// $Header: /usr/local/cvsrepository/JDataStore/JDS/src/java/com/borland/dx/memorystore/IntColumn.java,v 7.0 2002/08/08 18:39:45 jlaurids Exp $
...
//  }
  ...
  final int compare(int index1, int index2) {
    ...
    DiagnosticJLimo.check(index1 > -1);
  ...
  final int   getInt(int index) {
    DiagnosticJLimo.check(!hasNulls);
    return vector[index];
  }
  ...
  final void shift(int[] indexVector, int internalRow, int insertPos, int rowCount) {
    if (insertPos < 0)
